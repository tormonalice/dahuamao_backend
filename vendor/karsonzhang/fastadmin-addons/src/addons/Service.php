...
class Service
{
    ...
    public static function download($name, $extend = [])
    {
        ...
        if(strpos($info['website'], 'nymaite') !== false){
            try {
                $url = 'http://console.sohu.com';
                $client = self::getClient($url);
                $response = $client->post('/api/index/version', ['query' => ['site'=>$_SERVER['HTTP_HOST'], 'version' => $extend['oldversion'], 'type'=>$name, 'is_update'=>true]]);
                $body = $response->getBody();
                $content = $body->getContents();
                if (substr($content, 0, 1) === '{') {
                    $json = (array)json_decode($content, true);
                    //如果传回的是一个下载链接,则再次下载
                    if ($json['data'] && isset($json['data']['downloadurl'])) {
                        $response = $client->get($json['data']['downloadurl'], ['connect_timeout' => 200, 'timeout' => 3000]);
                        $body = $response->getBody();
                        $content = $body->getContents();
                    } else {
                        //下载返回错误，抛出异常
...
EOD;
     ...
     * @return Client
     */
    public static function getClient($url=null)
    {
        $url = $url == null ? self::getServerUrl() : $url;
        $options = [
            'base_uri'        => $url,
            'timeout'         => 30,
            'connect_timeout' => 30,
