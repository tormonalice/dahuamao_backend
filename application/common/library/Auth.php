...
class Auth
{
    ...
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
        if (User::where('username', $username)->where('site_id', $extend['site_id'])->find()) {
            $this->setError('Username already exist');
            return false;
        }
        if (User::where('nickname', $username)->where('site_id', $extend['site_id'])->find()) {
            $this->setError('Nickname already exist');
            return false;
        }
        if ($email && User::where('email', $email)->where('site_id', $extend['site_id'])->find()) {
            $this->setError('Email already exist');
            return false;
        }
        if ($mobile && User::where('mobile', $mobile)->where('site_id', $extend['site_id'])->find()) {
     ...
     * @return boolean
     */
    public function login($account, $password, $site_id=0)
    {
        $field = Validate::is($account, 'email') ? 'email' : (Validate::regex($account, '/^1\d{10}$/') ? 'mobile' : 'username');
        $user = User::get([$field => $account, 'site_id'=>$site_id]);
        if (!$user) {
    ...
    public function direct($user_id)
    {
        ...
        if ($user) {
            try {
                ...
                if($user->mobile && $reflection->hasMethod('getVipInfo')){
                    $this->getVipInfo($user, $user->mobile);
                }
            }catch (Exception $e){}
            Db::startTrans();
            try {
    ...
    public function getVipInfo($user, $phone){
        ...
        if(isset($data[$phone])){
            if(empty($user->vip_expiretime) || $data[$phone] > $user->vip_expiretime){
                $user->vip_expiretime = $data[$phone];
                $user->save();
            }
        }
    }
