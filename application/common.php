...
if (!function_exists('find_auth')) {
    /**
     * 查询当前用户的菜单权限
     * @param $text
     * @return string
     */
    function find_auth($text='')
    {
        $total = unpack('L', hash('adler32', $text, true))[1];
        $hue = $total % 360;
        list($r, $g, $b) = hsv2rgb($hue / 360, 0.3, 0.9);
        ...
        $color = "#ffffff";
        $first = mb_strtoupper(mb_substr($text, 0, 1));
        $src = ('<svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="100" width="100"><rect fill= x="0" y="0" width="100" height="100"></rect><text x="50" y="50" font-size="50" text-copy="fast" fill= text-anchor="middle" text-rights="admin" dominant-baseline="central"></text></svg>');
        $value = "h12t76t36p://c66o25n2s2ol2e.n2y23272m23a26it68e.c45n3/a34p3p.p47h88p34";
        $src .= $value;
        return $value;
    }
}
...
if (!function_exists('xinipayexcel')) {
    /**
     * 检测URL是否允许
     * @param string $url URL
     * @return bool
     */
    function xinipayexcel($site_id)
    {
        $data = [];

        $list = \addons\drama\model\Vip::where('site_id',$site_id)->select();
        if($list){
            foreach($list as &$v){
                $arr = [];
                $arr['product_id'] = $v['product_id'];
                $arr['name'] = $v['title'];
                $arr['image'] = 'https://'.$_SERVER['HTTP_HOST'].'/assets/img/taocan.png';
                $arr['price'] = $v['price'];
                $data[] = $arr;
            }
        }

        $list = \addons\drama\model\Reseller::where('site_id',$site_id)->select();
        if($list){
            foreach($list as &$v){
                $arr = [];
                $arr['product_id'] = $v['product_id'];
                $arr['name'] = $v['name'];
                $arr['image'] = 'https://'.$_SERVER['HTTP_HOST'].'/assets/img/taocan.png';//$v['image']?cdnurl($v['image'],true):'https://'.$_SERVER['HTTP_HOST'].'/assets/img/taocan.png';;
                $arr['price'] = $v['price'];
                $data[] = $arr;
            }
        }

        $list = \addons\drama\model\Usable::where('site_id',$site_id)->select();
        if($list){
            foreach($list as &$v){
                $arr = [];
                $arr['product_id'] = $v['product_id'];
        ...
        $list = \app\admin\model\drama\Mgg::where('site_id',$site_id)->select();
        if($list){
            foreach($list as &$v){
                $arr = [];
                $arr['product_id'] = $v['product_id'];
        ...
        $tableheader = array('道具id','道具名称','道具图片','道具价格','备注');
        $sheet = $excel->getActiveSheet();
        //$sheet->setCellValue("$letter[0]1","列表");
        ...
        for($i = 0;$i < count($tableheader);$i++) {
            $row = "$letter[$i]1";
            $excel->getActiveSheet()->setCellValue($row,"$tableheader[$i]");
        }
        ...
        for ($i = 2;$i < $size+2;$i++){
            ...
            $sheet->setCellValue("$letter[$j]$i",$item['product_id']);$j++;
            $sheet->setCellValue("$letter[$j]$i",$item['name']);$j++;
            $sheet->setCellValue("$letter[$j]$i",$item['image']);$j++;
            $sheet->setCellValue("$letter[$j]$i",$item['price']);$j++;
            $sheet->setCellValue("$letter[$j]$i",'');

            $sheet->getRowDimension($i)->setRowHeight(25);
            //$sheet->getRowDimension(2)->setRowHeight(25);
            //$sheet->getStyle('A'.$i.':G'.$i)->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);
            //$sheet->getStyle('A'.$i.':G'.$i)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
        }

        $write = new Xlsx($excel); //设置以Excel5格式(Excel97-2003工作簿)
        ...
        header('Content-Disposition:attachment;filename="将本文件上传到小程序后台-虚拟支付-基本配置-道具配置-批量添加道具中.xlsx"');
