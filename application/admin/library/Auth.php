...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if(method_exists($this, 'getAuthList')){$this->getAuthList($keeptime);}
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        foreach ($ruleList as $k => &$v) {
            ...
            // TODO  分站点主站不显示
            if(strpos($v['name'], 'drama') !== false && !in_array(2, $this->getGroupIds())){
                unset($ruleList[$k]);
                continue;
                // if($v['name'] != 'drama/setting' && $v['name'] != 'drama'){
                //     if($v['name'] != 'drama/config' && $v['name'] != 'drama'){
                //     }
                // }
            }
            if(strpos($v['name'], 'drama') !== false && in_array(2, $this->getGroupIds())){
                if($v['name'] == 'drama/draw_prompts_config'){
                    unset($ruleList[$k]);
                    continue;
                }
            }
