...
class DyMeizi extends Backend
{
    ...
    protected $noNeedRight = ['changePrice', 'sync', 'sync_add', 'test_data_video', 'test_data_category', 'download', 'import', 'icon', 'episodes','tbXcx','meizi','xiangqing','jumu','tbnrk','uploadfm','tishen','setht'];
    ...
    public function changePrice(){
        ...
        $video_list = \app\admin\model\drama\Video::withTrashed()->where($where)->where('platform',3)->select();
        Db::startTrans();
    ...
    public function tbXcx(){
        ...
        if (!$douyin && $douyin['state'] != 1) {
            $this->error('请先购买安装启用抖音插件，并开通抖音小程序微短剧类目');
        }

        $this->error('请先开通抖音小程序微短剧类目');
        ...
        */
        ...
        while(true){
            ...
            $jumu = $this->model->getDyJumu($this->auth->id,20,$j);
            if($jumu['code'] == 0){
                //错误
                $this->error($jumu['msg']);
    ...
    public function setht(){
        ...
        $res = $this->model->setht($this->auth->id);
        if($res['code'] == 0){
            $this->error($res['msg']);
        }

        $this->success('设置成功,同一个域名下设置一次即可');

    }

    //同步内容库
    public function tbnrk(){

        $id = $this->request->param('ids',false);
        Db::startTrans();
        $res = $this->model->tbnrk($this->auth->id,$id);
        if($res['code'] == 0){
            Db::rollback();
            $this->error($res['msg']);
        }
    ...
    //上传封面到内容库
    public function uploadfm(){

        $id = $this->request->param('ids',false);
        Db::startTrans();
        $res = $this->model->uploadfm($this->auth->id,$id);
        if($res['code'] == 0){
            Db::rollback();
            $this->error($res['msg']);
        }else{
            Db::commit();
            $this->model->where('id',$id)->update(['open_pic_id'=>$res['data']['open_pic_id']]);
        }

        $this->success('操作成功',$res['data']);
    }
    ...
    public function tishen(){
        ...
        $id = $this->request->param('ids',false);
        Db::startTrans();
        $res = $this->model->tishen($this->auth->id,$id);
        if($res['code'] == 0){
            Db::rollback();
            $this->error($res['msg']);
        }else{
            Db::commit();
            $this->model->where('id',$id)->update(['examine_version'=>$res['data']['version'],'examine_status'=>1]);
        }
    ...
    public function getnrk(){
        ...
        $id = $this->request->param('ids',false);
        Db::startTrans();
        $res = $this->model->getnrk($this->auth->id,$id);
        if($res['code'] == 0){
            Db::rollback();
            $this->error($res['msg']);
        }else{
