...
class Category extends Model
{
    ...
    public static function getCategoryDetail($site_id) {
        $category = self::where(['site_id' => $site_id, 'pid' => 0, 'status' => 'normal'])->order('weigh desc, id asc')->order('id', 'desc')->select();

        return $category;
    }


    public static function getCategoryList($id, $site_id)
    {
        $category = self::where(['site_id' => $site_id, 'pid' => $id, 'status' => 'normal'])->with('children.children.children')->select();

        return $category;
    }

    private static function getAllChirdrenCategory($pid, $type = 'video', $status = 'normal')
    {
        $where = [
            'type' => $type,
            'status' => $status,
    ...
    {
        return $this->hasMany(\addons\drama\model\Category::class, 'pid', 'id')->where('status', 'normal')->order('weigh desc, id asc');
    }
