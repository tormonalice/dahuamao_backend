...
class UserWalletApply extends Model
{
    ...
    // 表名,不含前缀
    protected $name = 'drama_user_wallet_apply';
    // 自动写入时间戳字段
     ...
     */
    public static function apply($type, $money, $platform)
    {
        $user = User::info();
        ...
        // 检查提现账户信息
        $bank = \addons\drama\model\UserBank::info($type, false);
        ...
        switch ($type) {
            case 'wechat':
                $applyInfo = [
                    '真实姓名' => $bank['real_name'],
                    '微信账户'  => $bank['card_no'],
                    '收款码'  => $bank['image'] ? cdnurl($bank['image'], true) : '',
                ];
                break;
            case 'alipay':
                $applyInfo = [
    ...
    private static function handleTransfer($apply)
    {
        ...
        try {
            $pay = new \addons\drama\library\PayService($type, $platform, '', 'transfer');
    ...
    public static function getWithdrawConfig()
    {
        ...
        $config = \addons\drama\model\Config::where('name', 'withdraw')
