...
class User extends Model
{
    ...
    public function getAvatarAttr($value, $data)
    {
        if (!$value) {
            ...
            $config = json_decode(\addons\drama\model\Config::get(['name' => 'user'])->value, true);
            $value = $config['avatar'];
        }
    ...
    public static function usable($usable, $user, $type = '', $type_id = 0, $memo = '', $ext = [])
    {
        ...
        // 判断积分
        if ($usable === 0) {
            new Exception('请输入正确的剧场积分');
        }
        $before = $user->usable;
        $after = $user->usable + $usable;
        if ($after < 0) {
            new Exception('可用剧场积分不足');
        }
        try {
            // 更新会员余额信息
    ...
    {
        return $this->hasMany(\addons\drama\model\User::class, 'parent_user_id')->field('id,nickname,avatar,parent_user_id');
    }
