...
class Alioss extends Controller
{
    ...
    public function params()
    {
        ...
        $auth = new \addons\uploads\library\AliAuth();
    ...
    public function upload($isApi = false)
    {
        ...
        if ($chunkid) {
            ...
            try {
                ...
                $this->error("上传失败(1002):".$e->getMessage());
            }
            $checkDeleteFile($attachment, $upload);
    ...
    public function notify()
    {
        ...
        $attachment = Attachment::where('site_id', $site_id)->where('url', $url)->where('storage', 'alioss')->find();
        if (!$attachment) {
            $params = array(
                'category'    => $category,
                'site_id'     => (int)$site_id,
