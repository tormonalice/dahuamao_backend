...
(function (window, undefined) {
    ...
    function _clearMsWord(html, htmlTags) {
        html = html.replace(/<meta[\s\S]*?>/ig, '')
            ...
            .replace(/<w:([^>]+)>[\s\S]*?<\/w:$1>/ig, '')
            .replace(/<o:([^>]+)>[\s\S]*?<\/o:$1>/ig, '')
        ...
        select: function (hasDummy) {
            ...
            if (_IERANGE) {
                ...
                if (doc !== document) {
                    ...
                    if(sc.children && sc.children.length>1 && sel.focusOffset>1){
                        pos = K(sc.children[sel.focusOffset-1]).pos();
                    }
    ...
    self.plugin.media = {
        edit: function () {
            var html = [
                ...
                '</div>',
                '</div>',
                '<div class="ke-dialog-row ke-clearfix" style="display:none;">',
...
KindEditor.plugin('graft', function (K) {
    ...
    self.plugin.graftDialog = function (options) {
        ...
        var html = [
            ...
            '<input type="file" class="upload" id="J_canvas_bg" accept="image/gif,image/jpeg,image/png,image/jpg,image/bmp,image/webp,image/webp"/>',
            '</div>',
            '</div>',
            '<div class="sectionBar">',
