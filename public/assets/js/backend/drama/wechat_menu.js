                        ...
                        switch (opttype) {
                            case 'delete':
                                that.$confirm('此操作将删除菜单, 是否继续?', '提示', {
                                    ...
                                    Fast.api.ajax({
                                        url: 'drama/wechat_menu/del/ids/' + id,
                                        loading: true,
                                        type: 'POST',
                                    }, function (ret, res) {
                                        that.getList()
                            ...
                            case 'status':
                                that.$confirm('确认发布当前菜单？公众号底部菜单有延迟生效时间，您可稍等或重新关注查看', '提示', {
                                    ...
                                    Fast.api.ajax({
                                        url: `drama/wechat_menu/publish?id=${id}`,
                                        loading: true,
                                    }, function (ret, res) {
                                        that.getList();
                                    })
                                ...
                                break;
                            case 'create':
                                Fast.api.open("drama/wechat_menu/add", "新建", {
                                    callback(data) {
                                        that.getList();
                                    }
                                });
                            ...
                            case 'copy':
                                Fast.api.ajax({
                                    url: `drama/wechat_menu/copy?id=${id}`,
                                    loading: true,
                                }, function (ret, res) {
                                    that.getList();
                                })
                                break;
                            case 'edit':
                                Fast.api.open(`drama/wechat_menu/edit?id=${id}`, "编辑", {
                                    callback(data) {
                                        that.getList();
                                    }
                                });
            ...
            var wechatMenu = new Vue({
                ...
                methods: {
                    ...
                    getoptions() {
                        let that = this;
                        Fast.api.ajax({
                            url: 'drama/wechat_material/select',
                            loading: true,
                            type: 'GET',
                            data: {
                    ...
                    createTemplate() {
                        let that = this;
                        Fast.api.open(`drama/wechat_material/add?type=${that.rightData.media_type}`, '创建', {
                            callback(data) {
                                that.getoptions();
                            }
                        })
                    ...
                    choosePath() {
                        ...
                        parent.Fast.api.open("drama/link/select?multiple=" + multiple, "选择路径", {
                    ...
                    subData(type) {
                        ...
                        if (that.optType == 'edit') {
                            if (type == 'publish') {
                                urlsub = 'drama/wechat_menu/edit?id=' + that.edit_id + '&act=' + type
                            } else {
                                urlsub = 'drama/wechat_menu/edit?id=' + that.edit_id
                            }
                        } else {
                            if (type == 'publish') {
                                urlsub = 'drama/wechat_menu/add?act=' + type
                            } else {
                                urlsub = 'drama/wechat_menu/add'
                            }
                        }
                        // return false;
                        Fast.api.ajax({
