            ...
            var materialIndex = new Vue({
                ...
                methods: {
                        ...
                        switch (opttype) {
                            case 'delete':
                                that.$confirm('此操作将删除菜单, 是否继续?', '提示', {
                                    ...
                                    Fast.api.ajax({
                                        url: 'drama/wechat_material/del/ids/' + id,
                                        loading: true,
                                        type: 'POST',
                                    }, function (ret, res) {
                                        that.getlistData()
                                ...
                                break;
                            case 'create':
                                Fast.api.open(`drama/wechat_material/add?type=${that.activeName}`, "新建", {
                                    callback(data) {
                                        that.getlistData();
                                    }
                                });
                                break;
                            case 'edit':
                                Fast.api.open(`drama/wechat_material/edit?id=${id}&type=${that.activeName}`, "编辑", {
                                    callback(data) {
                                        that.getlistData();
                                    }
                                });
                    ...
                    getlistData() {
                        var that = this;
                        Fast.api.ajax({
                            url: 'drama/wechat_material/index',
                            loading: true,
                            type: 'GET',
                            data: {
                                offset: that.offset,
                        ...
                        if (type == 'yes') {
                            this.$refs[issub].validate((valid) => {
                                if (valid) {
                                    ...
                                    if (this.optType != 'add') {
                                        Fast.api.ajax({
                                            url: 'drama/wechat_material/edit?id=' + that.text_id,
                                            loading: true,
                                            type: "POST",
                                            data: {
