            ...
            var replyIndex = new Vue({
                ...
                methods: {
                    getFormData() {
                        let that = this;
                        Fast.api.ajax({
                            url: "drama/wechat_auto_reply/index?type=" + that.activeName,
                            loading: true,
                            type: 'GET',
                        }, function (ret, res) {
                        ...
                        switch (opttype) {
                            case 'delete':
                                that.$confirm('此操作将删除菜单, 是否继续?', '提示', {
                                    ...
                                    Fast.api.ajax({
                                        url: 'drama/wechat_auto_reply/del/ids/' + id,
                                        loading: true,
                                        type: 'POST',
                                    }, function (ret, res) {
                                        that.getList()
                                ...
                                break;
                            case 'create':
                                Fast.api.open("drama/wechat_auto_reply/add", "新建", {
                                    callback(data) {
                                        that.getList();
                                    }
                                });
                                break;
                            case 'edit':
                                Fast.api.open("drama/wechat_auto_reply/edit?id=" + id, "编辑", {
                                    callback(data) {
                                        that.getList();
                                    }
                                });
                    ...
                    getList() {
                        var that = this;
                        Fast.api.ajax({
                            url: 'drama/wechat_auto_reply/index',
                            loading: true,
                            type: 'GET',
                            data: {
                                offset: that.offset,
                    ...
                    createTemplate() {
                        let that = this;
                        Fast.api.open(`drama/wechat_material/add?type=${that.detailForm.type}`, '创建', {
                            callback(data) {
                                that.getoptions();
                            }
                        })
                        ...
                        if (type == 'yes') {
                            this.$refs[issub].validate((valid) => {
                                if (valid) {
                                    ...
                                    Fast.api.ajax({
                                        url: 'drama/wechat_auto_reply/edit?id=' + that.activeName,
                                        loading: true,
                                        data: {
        ...
        detailInit: function (type) {
            var replyDetail = new Vue({
                ...
                methods: {
                        ...
                        if (type == 'yes') {
                            that.$refs[issub].validate((valid) => {
                                if (valid) {
                                    ...
                                    if (that.optType != 'add') {
                                        Fast.api.ajax({
                                            url: 'drama/wechat_auto_reply/edit?id=' + that.detail_id,
                                            loading: true,
                                            data: {
                                                data: JSON.stringify(subData)
                                            }
                                        ...
                                        Fast.api.ajax({
                                            url: 'drama/wechat_auto_reply/add',
                                            loading: true,
                                            type: "POST",
                                            data: {
                                                data: JSON.stringify(subData)
                    ...
                    createTemplate() {
                        let that = this;
                        Fast.api.open(`drama/wechat_material/add?type=${that.detailForm.type}`, '增加文本', {
                            callback(data) {
                                that.getoptions();
                            }
                        })
