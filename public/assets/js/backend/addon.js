define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template', 'cookie'], function ($, undefined, Backend, Table, Form, Template, undefined) {
    ...
    var Controller = {
        index: function () {
            ...
            // 点击抖音小程序安装
            $(document).on("click", ".btn-tishi", function () {
                Layer.alert('因成本问题此插件需付费升级，请联系官方按需购买。', {icon: 7});
            });
        ...
        api: {
            formatter: {
                ...
                version: function (value, row, index) {
                    if(row.name == 'drama'){
                        return row.addon && row.addon.version != row.version ? '<a href="https://doc.weixin.qq.com/doc/w3_AToAgQZxAO8R1LFXO1gTUK0vKv0wP?scode=AEIAIgdXAAoHEhrrxUAToAgQZxAO8" target="_blank"><span class="releasetips text-primary" data-toggle="tooltip" title="' + __('New version tips', row.version) + '">' + row.addon.version + '<i></i></span></a>' : row.version;
                    }else{
